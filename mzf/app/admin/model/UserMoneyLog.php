...
class UserMoneyLog extends model
{
    ...
    public static function onBeforeInsert($model): void
    {
        ...
        // 用户当前余额（已由获取器转为元）
        $beforeYuan = $user->money;
        $model->before = $beforeYuan;

        // 计算新余额（元 + 元）
        $changeYuan = (float) $model->money;
        $afterYuan = bcadd($beforeYuan, $changeYuan, 2);

        // 更新用户余额
        $user->money = $afterYuan;
        $user->save();

        $model->after = $afterYuan;
    }
