    ...
    public function index(): void
    {
        ...
        if ($this->auth->isLogin()) {
            ...
            foreach ($userMenus as $item) {
                // menu_dir（分类）与顶级 menu（如「API配置」直达页）都进会员中心菜单
                if ($item['type'] == 'menu_dir' || $item['type'] == 'menu') {
                    $menus[] = $item;
        ...
        $this->success('', [
            'site'             => [
                ...
                'homeTemplate' => \app\common\library\theme\HomeTemplate::active(),
                'siteLogo'     => (string) get_sys_config('site_logo'),
                'favicon'      => (string) get_sys_config('site_favicon'),
                'homeEnable'   => (bool) get_sys_config('home_enable'),
                'webmasterQq'  => (string) get_sys_config('webmaster_qq'),
            ],
