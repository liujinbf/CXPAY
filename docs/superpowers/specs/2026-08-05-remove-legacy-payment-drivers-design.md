...
MySQL 的建表 DDL 会触发隐式提交，因此迁移明确分成两个阶段。

**阶段一：幂等建表**

- 使用 `CREATE TABLE IF NOT EXISTS` 创建归档表；
- 校验归档表结构和 `original_channel_id` 唯一索引；
- 此阶段不修改活动通道、套餐或轮询组数据。

**阶段二：事务性归档与删除**

迁移脚本默认只执行 dry-run，输出待处理通道 ID、驱动标识和数量，不修改数据。只有显式传入 `--apply` 才进入事务并执行：

1. 再次锁定并读取五类待删除活动通道；
2. 按 `original_channel_id` 幂等写入归档表；
3. 删除 `cx_poll_group_channel` 中对应 `channel_id` 的关联；
4. 删除 `cx_pay_channel` 中对应活动通道；
5. 清理当前套餐表 `allowed_channels` 字段中的五个驱动标识，并保持该字段原有存储格式及其他值顺序；
6. 校验活动通道残留数为零；
7. 输出归档数、关联删除数、活动通道删除数、套餐更新数及残留数；
8. 任一步失败则回滚阶段二的全部 DML。

迁移必须可重复执行。第二次执行不得新增重复归档，也不得报错；dry-run 和 `--apply` 均应得到零待处理结果。

如果部署数据库不存在轮询组关联表或套餐表，迁移应记录“表不存在，跳过”，而不是失败。活动通道表和归档表是强制依赖，缺失时必须停止。
...
- 迁移执行前必须备份数据库；
- 默认 dry-run，必须显式使用 `--apply` 才允许修改数据；
- 阶段二事务失败时保留原活动通道，不允许部分删除；
