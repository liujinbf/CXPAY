...
final class ProviderClient
{
    ...
    public function createAuthSession(array $config): array
    {
        return $this->request('POST', '/v1/auth-sessions', $config, [
            'reference' => 'cxpay-channel-' . (string)($config['channel_id'] ?? 'unknown'),
        ]);
    }
    ...
    private function request(string $method, string $path, array $config, array $payload = []): array
    {
        ...
        if (UrlGuard::resolve($baseUrl) === null) {
            throw new RuntimeException('云监控服务地址不可用或不是公网 HTTP(S) 地址');
        }
        ...
        if ($response->getStatusCode() < 200 || $response->getStatusCode() >= 300) {
            throw new RuntimeException('云监控接口暂时不可用，HTTP ' . $response->getStatusCode());
        }
        ...
        if ($responseSignature === '' || !$verified) {
            throw new RuntimeException('云监控响应签名无效');
        }
        $data = json_decode($body, true);
        if (!is_array($data)) {
            throw new RuntimeException('云监控响应格式错误');
        }
        return $data;
    }
}
