...
### 2. 云端控制面（CXPAY Cloud）与代理商体系协同：
- **全网资产与商业授权统一管理**：统一管理站点授权（`cloud_site_licenses`）、插件目录与拿货价格体系；
- **OEM 代理商工作台协同模式**：
  - 云端提供权威代理商管理 API 服务（`POST /api/agent/v1/*`，包含配额核算、授权生成、插件划拨与双分录账本）；
  - 代理商可在具备代理资质的主站后台直接使用 **「👑 OEM 代理加盟中心」** 可视化工作台，一键下发客户子站点、查询站点状态、免费域名改绑与划拨付费插件；
- **高等级安全保障**：所有发证与划拨指令均通过 Ed25519 密钥签名向官方云端验证后生效，资金与配额安全闭环。
